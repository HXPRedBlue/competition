...
class ECGDataset(Dataset):
    def __init__(self, data_path, train=True) -> None:
        super().__init__()
        labels = 12
        train_lines = open(f'{data_path}/trainreference.csv').read().splitlines()
        ...
        val_lines = open(f'{data_path}/valreference.csv').read().splitlines()
        val_l_ori = np.array([
                        np.sum(np.eye(labels)[np.array(i.strip().split(',')[1:]).astype(int) - 1], axis=0)
                        for i in val_lines
                        ], dtype=np.float32)
        val_names = [f"{data_path}/Train/{i.strip().split(',')[0]}" for i in val_lines]
        val_s_ori = [sio.loadmat(x)['ecgdata'] for x in val_names]


        train_s_ori = np.array(train_s_ori, dtype=np.float32)
        val_s_ori = np.array(val_s_ori, dtype=np.float32)
